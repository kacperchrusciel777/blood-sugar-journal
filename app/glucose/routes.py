...
from app import db
from app.glucose.forms import GlucoseForm, DeleteForm
...
@login_required
def dashboard():
    entries = GlucoseEntry.query.filter_by(user_id=current_user.id).order_by(GlucoseEntry.timestamp.asc()).all()
    glucose_data = [
        {"date": entry.timestamp.strftime('%Y-%m-%d'), "glucose": entry.glucose}
        for entry in entries
    ]
    delete_form = DeleteForm()
    return render_template('glucose/dashboard.html', entries=entries, glucose_data=glucose_data, delete_form=delete_form)
...
def delete_entry(id):
    ...
    db.session.commit()
    flash('Glucose level entry has been deleted.', 'danger')
    return redirect(url_for('glucose.dashboard'))
